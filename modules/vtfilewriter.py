...
write_data() {
    ...
    llOwnerSay(llGetScriptName() + " writing " + (string) out_key + " up to " + (string) end_key);
    integer index;
    ...
    if (SCRIPT_NUMBER == LAST_SCRIPT_NUMBER) {
        ...
        llOwnerSay("SIGNALLING LOADING_DONE " + (string) keyCount);
...
default {
    ...
    link_message(integer sender_num, integer num, string str, key id) {
        ...
        if (SCRIPT_NUMBER == 0) {
            llLinksetDataReset();
            llOwnerSay("SCRIPT 0 Resetting LSD");
        }
        write_data();
    }
}
